...
//		});
		...
		myModel.save({title : this.input.val()}, {
		    success: function (model, response) {
		      var myKey = model.get("key");
		      var myTitle = model.get("title");
		      
		      var myHtml = '<input class= "toggle-all"id='+myKey+' type="checkbox">'+
		    	            '<li id='+myKey+'><div class="Myli" id='+myKey+'><h5 class="myDataTitle">'+myTitle+'</h5>'+
...
//				"<li>" + this.input.val()+"<button id='<%=person.key%>' class='deleteEntry' {console.log(JSON.parse(person.data))><i class='fa fa-times' aria-hidden='true'></i></button></form></div></li>")
//		
//				
...
//		app.TodoView.render();
	 ...
	 deleteList : function(e) {
			 ...
			 var myModel = this.collection.where({key: id});
						 ...
						 $.ajax({
								...
								success:function(data){
									...
									var myData = JSON.parse(data);
									 $("#"+key).children(".textEdit").hide();
									 var val = $("#"+key).children(".textEdit").val();	
									 console.log(val);
									 
									
									 $("#"+key).children(".myDataTitle").html(val);
									 
									 $("#"+key).children(".myDataTitle").show();
